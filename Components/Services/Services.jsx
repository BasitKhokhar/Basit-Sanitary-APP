...
const Services = () => {
  ...
  if (loading) {
    return (
      <View style={[styles.loaderContainer, { backgroundColor: colors.cardsbackground }]}>
        <Loader />
      </View>
    );
  }
  ...
  return (
    <View style={{ paddingBottom: 110, backgroundColor: colors.bodybackground }}>
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.maincontainer}>
            <View style={[styles.servicescontainer, { backgroundColor: colors.cardsbackground }]}>
              ...
              <Text style={[styles.subHeading, { color: colors.text }]}>Our Plumbers</Text>
            </View>
          </View>
        }
        renderItem={({ item }) => (
          <View style={styles.cardscontainer}>
            <View style={[styles.card, { backgroundColor: colors.cardsbackground, borderColor: colors.border }]}>
              ...
              <View style={styles.cardright}>
                ...
                <Text style={[
                  styles.status,
                  item.status.toLowerCase().includes('available') ? { color: colors.primary } : { color: colors.error }
                ]}>
                  {item.status}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: "row", gap: 20, padding: 10, marginHorizontal: 15, borderWidth: 1, borderRadius: 8, alignItems: 'center' },
  plumberImage: { width: 100, height: 100, borderRadius: 8 },
  cardright: { flex: 1 },
  cardTitle: { fontSize: 16, fontWeight: 'bold' },
  cardText: { fontSize: 14 },
  status: { fontWeight: "800", marginTop: 5 }
});
